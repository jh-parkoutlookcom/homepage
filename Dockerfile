# build stage for react
FROM node:latest AS react-builder
WORKDIR /app
COPY frontend/ .
RUN npm install && npm run build

# build stage for rust
FROM rust:latest AS rust-builder
RUN apt-get update && apt-get install -y \
    build-essential \
    wget \
    unzip \
    libssl-dev \
    ca-certificates \ 
    protobuf-compiler
# oracle client installation
RUN mkdir -p /opt/oracle \
    && cd /opt/oracle \
    && wget -q https://download.oracle.com/otn_software/linux/instantclient/2326100/instantclient-basic-linux.x64-23.26.1.0.0.zip  \
    && wget -q https://download.oracle.com/otn_software/linux/instantclient/2326100/instantclient-sdk-linux.x64-23.26.1.0.0.zip  \
    && unzip -q instantclient-basic-linux.x64-23.26.1.0.0.zip  \
    && unzip -qo instantclient-sdk-linux.x64-23.26.1.0.0.zip \
    && rm -f *.zip \
    && echo /opt/oracle/instantclient_23_26 > /etc/ld.so.conf.d/oracle-instantclient.conf \
    && ldconfig

# tnsnames.ora 파일 생성
RUN echo "HOMEPAGE = \
  (DESCRIPTION = \ 
    (ADDRESS_LIST = \ 
      (ADDRESS = (PROTOCOL = TCP)(HOST = db.yuiop.org)(PORT = 1521)) \
    ) \ 
    (CONNECT_DATA = \ 
      (SERVICE_NAME = HOMEPAGE) \
    ) \
  )" > /opt/oracle/instantclient_23_26/network/admin/tnsnames.ora

# Oracle 환경변수를 시스템 전체에 적용
RUN echo '#!/bin/sh' > /etc/profile.d/oracle.sh \
    && echo 'export ORACLE_HOME=/opt/oracle/instantclient_23_26' >> /etc/profile.d/oracle.sh \
    && echo 'export LD_LIBRARY_PATH=$ORACLE_HOME:$LD_LIBRARY_PATH' >> /etc/profile.d/oracle.sh \
    && echo 'export PATH=$ORACLE_HOME:$PATH' >> /etc/profile.d/oracle.sh \
    && chmod +x /etc/profile.d/oracle.sh

WORKDIR /app
COPY . .
RUN cargo build --release -p web-server

# production stage
FROM debian:stable-slim AS production
RUN apt-get update && apt-get install -y \
    ca-certificates \
    wget \
    unzip \
    libssl-dev

WORKDIR /app 

COPY --from=rust-builder /opt/oracle /opt/oracle
COPY --from=rust-builder /etc/ld.so.conf.d/oracle-instantclient.conf /etc/ld.so.conf.d/oracle-instantclient.conf
RUN ldconfig
COPY --from=rust-builder /etc/profile.d/oracle.sh /etc/profile.d/oracle.sh
RUN chmod +x /etc/profile.d/oracle.sh
COPY --from=rust-builder /app/target/release/web-server /app/web-server
COPY --from=react-builder /app/dist /app/frontend
EXPOSE 8080 30015
CMD ["/app/web-server"]
